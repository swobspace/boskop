...
RSpec.describe AccessType, type: :model do
  ...
  it "should get plain factory working" do
    f = FactoryBot.create(:access_type)
    g = FactoryBot.create(:access_type)
  ...
  it "to_s returns value" do
    f = FactoryBot.create(:access_type, name: 'SuperDSL')
    expect("#{f}").to match ("SuperDSL")
  end

  [:line].each do |what|
    it "should not destroyable if dependent #{what} exist" do
      at   = FactoryBot.create(:access_type)
      subj = FactoryBot.create(what, access_type: at)
      expect {
        at.destroy
