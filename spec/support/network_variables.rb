shared_context "network variables" do
  ...
  let!(:addr_paris) { FactoryBot.create(:address, ort: 'F_Paris', addressfor: paris) }
  let!(:addr_london) { FactoryBot.create(:address, ort: 'GB_London', addressfor: london) }
  let!(:addr_berlin) { FactoryBot.create(:address, ort: 'D_Berlin', addressfor: berlin) }
