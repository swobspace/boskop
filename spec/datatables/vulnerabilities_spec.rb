...
RSpec.describe VulnerabilitiesDatatable, type: :model do
  ...
  describe "with search:198.51.100" do
    ...
    it { expect(parse_json(subject, "data/0")).to eq(vuln2array(vuln13)) }
    it { expect(parse_json(subject, "data/1")).to eq(vuln2array(vuln12)) }
    it { expect(parse_json(subject, "data/2")).to eq(vuln2array(vuln11)) }
    it { expect(parse_json(subject, "data/3")).to eq(vuln2array(vuln23)) }
    it { expect(parse_json(subject, "data/4")).to eq(vuln2array(vuln22)) }
  end 
end
